...
public class CVTest {
	...
	public void testSetId() {
		int id = 1;
		cv.setId(id);
		Assert.assertTrue(id == cv.getId());
	}

	@Test
	public void testSetCvContentMap() {
		Map<String, List<Word>> cvContentMap = new HashMap<String, List<Word>>();
		List<Word> tempList = new ArrayList<Word>();
		tempList.add(new Word("java"));
		cvContentMap.put("skills", tempList);
		cv.setCvContentMap(cvContentMap);
		Assert.assertEquals(cvContentMap, cv.getCvContentMap());
	}

	@Test
	public void testToString() {
		CV tempCV = CV.fromString(content);
		Assert.assertEquals(tempCV.toString(), content);
	}

	@Test
	public void testFromString() {
		CV tempCV = CV.fromString(content);
		Assert.assertEquals(tempCV.getId(), CV.fromString(content).getId());
	}
